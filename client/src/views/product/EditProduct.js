...
import { isEmpty, cloneDeep, isUndefined, rest } from 'lodash';
...
const AddProduct = () => {
  ...
  const getSingleProduct = () => {
    axios
      .get(CONSTANTS.BASE_URL + 'api/product/' + id)
      .then(response => {
        ...
        if (!isEmpty(productData.product_image_big_url))
          setBigImageFiles(
            Array.from(productData.product_image_big_url).map(
              image => CONSTANTS.BASE_URL + image
            )
          );
        if (productData.product_image_small_url)
          setSmallImageFile(
            CONSTANTS.BASE_URL + productData.product_image_small_url
          );
  ...
  const getVarientPrices = () => {};
  const getUnitPrice = prices => {
    let unitPrice = prices[0].unit_price;
    for (let price in prices) {
      if (price.unit_price > unitPrice) unitPrice = price.unit_price;
    }
    return unitPrice;
  };
  const getQuantity = prices => {
    return prices[0].quantity;
  };
  const getSKU = prices => {
    let sku = '';
    if (!isEmpty(prices[0].sku)) sku = prices[0].sku;
    return sku;
  };
  const checkDiscount = prices => {
    if (!isEmpty(prices[0].discount_amount)) return true;
    return false;
  };
  const getVarientforInit = prices => {
    if (prices.length > 1)
      return prices.map(price => {
        return {
          _id: price._id,
          varient_value: price.varient_value,
  ...
  const getDefaultId = prices => {
    if (prices.length == 1) return prices[0]._id;
    return null;
  };
  ...
  const page = categories && productData && flashs && colors && attributes && (
    <Page className={classes.root} title="Edit Product">
      <Container maxWidth="lg">
        <Formik
          initialValues={{
            //id of the default price varient
            defaultVarientId: getDefaultId(productData.prices),
            language: 'en',
            category_id: productData.category_id,
            brand_id: productData.brand_id ? productData.brand_id : '',
            unit: productData.unit ? productData.unit : '',
            minimum_purchase_quantity: productData.minimum_purchase_quantity
              ? productData.minimum_purchase_quantity
              : '',
            tags: !isEmpty(productData.tags) ? productData.tags.join() : '',
            colorArray: productData.color_array ? productData.color_array : [],
            attributeArray: productData.attribute_array
              ? productData.attribute_array
              : [],
            valueObject: productData.attribute_value_object
              ? productData.attribute_value_object
              : [],
            //TODO:
            unit_price: getUnitPrice(productData.prices),
            quantity: getQuantity(productData.prices),
            sku: getSKU(productData.prices),
            hasDiscount: checkDiscount(productData.prices),
            discount: productData.prices[0].discount_amount
              ? productData.prices[0].discount_amount
              : '',
            discount_type: productData.prices[0].discount_type
              ? productData.prices[0].discount_type
              : 'flat',
            discount_date_from: productData.prices[0].discount_range
              ? productData.prices[0].discount_range.from
                ? productData.prices[0].discount_range.from
                : new Date()
              : new Date(),
            discount_date_to: productData.prices[0].discount_range
              ? productData.prices[0].discount_range.to
                ? productData.prices[0].discount_range.to
                : new Date()
              : new Date(),
            varients: getVarientforInit(productData.prices),
            flat_rate:
              productData.shipping_config == 'flat_rate' ? true : false,
            free_shipping: productData.shipping_config == 'free' ? true : false,
            shipping_cost: productData.shipping_cost
              ? productData.shipping_cost
              : 0,
            product_quantity_multiply: productData.product_quantity_multiply
              ? productData.product_quantity_multiply
              : false,
            todays_deal: productData.todays_deal,
            stock_visible: productData.stock_visible,
            low_stock_warning: productData.low_stock_warning
              ? productData.low_stock_warning
              : 1,
            cash_on_delivery: productData.cash_on_delivery,
            publish: productData.publish,
            featured: productData.featured,
            product_video_url: productData.product_video_url,
            flash: productData.flash
              ? productData.flash
              : { flash_id: '', discount_amount: '', discount_type: '' },
            // todays_deal: false,

            // product_image_big_url: [],
            // product_image_small_url: '',
            // icon_url: '',
            resourceBundle: productData.resourceBundle,
            tab: 0
          }}
          ...
          onSubmit={values => {
            ...
            if (!isEmpty(values.varients)) {
              values.varients.forEach((varient, index) => {
                ...
                if (!isEmpty(varient._id))
                  data.append('prices[' + index + '][_id]', varient._id);
              ...
              if (!isEmpty(values.defaultVarientId))
                data.append('prices[0][_id]', values.defaultVarientId);
            ...
            if (!isUndefined(values.product_image_big_url))
            ...
            if (!isUndefined(values.product_image_small_url)) {
              data.append(
                'product_image_small',
            ...
            axios
              .put(CONSTANTS.BASE_URL + 'api/product/' + productData._id, data)
              .then(response => {
