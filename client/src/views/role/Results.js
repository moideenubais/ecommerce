...
const Results = ({
  ...
  const handleDelete = (role, index) => {
    axios
      .delete(CONSTANTS.BASE_URL + 'api/role/' + role._id)
      .then(response => {
        setconfirmOpen(false);
        role.openConfirm = false;
        roles[index] = role;
      ...
      <PerfectScrollbar>
        <Box minWidth={1050}>
          <Table>
            ...
            <TableBody>
              {roles.slice(0, limit).map((role, index) => (
                <TableRow
                  hover
                  key={role._id}
                  // selected={selectedCustomerIds.indexOf(role.id) !== -1}
                >
                  {/* <TableCell padding="checkbox">
                    <Checkbox
                      checked={selectedCustomerIds.indexOf(role.id) !== -1}
                  ...
                  <TableCell>{role.shop_id ? role.shop_id : 'None'}</TableCell>
                  {/* <TableCell>
                    {role.icon_url?<img className={classes.iconImage} src={CONSTANTS.BASE_URL+role.icon_url} alt="icon" />:"No Image"}
                  </TableCell> */}
                  {/* <TableCell>
                        <Switch
                          checked={role.featured}
                          onChange={()=>{onUpdateFeature(role, index)}}
                          name="checkedFeature"
                        />
                  </TableCell> */}
                  <TableCell align="right">
                    <IconButton
                      ...
                      onClick={() => {
                        navigate('/app/roles/editRole/' + role._id, {
                          replace: false
                        });
                      }}
                    >
                  ...
                  <TableCell>
                    <IconButton
                      ...
                      onClick={() => {
                        handleDeleteConfirm(role, index);
                      }}
                    >
                      <DeleteIcon />
                    </IconButton>
                    {confirmOpen ? (
                      <AlertDialog
                        open={role.openConfirm}
                        handleClose={() => {
                          handleConfirmclose(role, index);
                        }}
                        handleConfirm={() => {
