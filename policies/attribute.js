...
exports.invokeRolesPolicies = function () {
  acl.allow([
    {
      roles: ["getAllAttributes"],
      allows: [
        {
          resources: ["/api/attribute/"],
          permissions: ["get"],
        },
      ],
    },
    {
      roles: ["createAttribute"],
      allows: [
        {
          resources: ["/api/attribute/"],
          permissions: ["post"],
    ...
    {
      roles: ["updateAttributeValue"],
      allows: [
        {
          resources: ["/api/attribute/value/:id"],
          permissions: ["put"],
        },
      ],
    },
    {
      roles: ["deleteAttributeValue"],
      allows: [
        {
          resources: ["/api/attribute/value/:id"],
          permissions: ["delete"],
    ...
    {
      roles: ["getSingleAttribute"],
      allows: [
        {
          resources: ["/api/attribute/:id"],
          permissions: ["get"],
        },
      ],
    },
    {
      roles: ["updateAttribute"],
      allows: [
        {
          resources: ["/api/attribute/:id"],
          permissions: ["put"],
...
exports.isAllowed = async function (req, res, next) {
  ...
  try {
    ...
    if (!_.isEmpty(role)) {
      if (!_.isEmpty(role[0].routes))
        currentRoute = role[0].routes.find(
          (route) => route.route_name == "attribute"
        );
    }
    ...
    if (_.isEmpty(currentRoute))
      return res
        .status(404)
        .json({ msg: "The route with the name attribute not found." });
