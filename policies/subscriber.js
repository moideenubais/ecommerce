...
exports.invokeRolesPolicies = function () {
  acl.allow([
    {
      roles: ["getAllSubscribers"],
      allows: [
        {
          resources: ["/api/subscriber/"],
          permissions: ["get"],
        },
      ],
    },
    ...
    {
      roles: ["getSingleSubscriber"],
      allows: [
        {
          resources: ["/api/subscriber/:id"],
          permissions: ["get"],
        },
      ],
    },
...
exports.isAllowed = async function (req, res, next) {
  ...
  try {
    ...
    if (!_.isEmpty(role)) {
      if (!_.isEmpty(role[0].routes))
        currentRoute = role[0].routes.find(
          (route) => route.route_name == "subscriber"
        );
    }
    ...
    if (_.isEmpty(currentRoute))
      return res
        .status(404)
        .json({ msg: "The route with the name subscriber not found." });
