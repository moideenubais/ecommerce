...
exports.invokeRolesPolicies = function () {
  acl.allow([
    ...
    },
    {
      ...
      allows: [
        {
          resources: ["/api/product/productBulkUpload"],
          permissions: ["post"],
        },
      ],
    },
    {
      roles: ["getSingleProduct"],
      allows: [
        {
...
exports.isAllowed = async function (req, res, next) {
  ...
  try {
    ...
    if (!_.isEmpty(role)) {
      if (!_.isEmpty(role[0].routes))
        currentRoute = role[0].routes.find(
          (route) => route.route_name == "product"
        );
    }
    ...
    if (_.isEmpty(currentRoute))
      return res
        .status(404)
        .json({ msg: "The route with the name product not found." });
